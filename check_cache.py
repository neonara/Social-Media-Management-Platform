            ...
            if cached_data is not None:
                ...
                if key_type == "user_data":
                    ...
                    if isinstance(cached_data, dict):
                        ...
                        print(
                            f"  - Image: {'Yes' if cached_data.get('image') else 'No'}"
                        )
                    else:
...
def check_post_cache(user_id=None, post_id=None):
    ...
    user_cache_keys = ["user_posts", "user_drafts", "cm_posts", "pending_posts"]

    total_cached_posts = 0

    for post in posts:
        print(
            f"Post: {post.title[:50]}{'...' if len(post.title) > 50 else ''} (ID: {post.id})"
        )
        print(
            f"Status: {post.status} | Creator: {post.creator.email if post.creator else 'N/A'}"
        )
        ...
        for key_type in post_cache_keys:
            ...
            if cached_data is not None:
                ...
                if isinstance(cached_data, dict):
                    ...
                    print(
                        f"  - Creator: {cached_data.get('creator', {}).get('email', 'N/A') if cached_data.get('creator') else 'N/A'}"
                    )
        ...
        for key_type in user_cache_keys:
            ...
            if cached_data is not None:
                ...
                if isinstance(cached_data, list):
                    ...
                    if cached_data:
                        print(
                            f"  - Latest: {cached_data[0].get('title', 'N/A')[:30] if cached_data[0] else 'N/A'}..."
                        )
                else:
                    print(f"  - Type: {type(cached_data).__name__}")
            else:
...
def main():
    ...
    parser.add_argument(
        "--test", "-t", action="store_true", help="Run cache operations test"
    )
    parser.add_argument(
        "--clear-all", "-c", action="store_true", help="Clear all user cache"
    )
    parser.add_argument(
        "--redis-only", "-r", action="store_true", help="Check Redis cache only"
    )
    parser.add_argument(
        "--django-only", "-d", action="store_true", help="Check Django cache only"
    )
