...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            nargs="?",
            default=None,
            help="Optional user ID to initialize",
        )
    ...
    def initialize_user_cache(self, user_id):
        ...
        try:
            ...
            self.stdout.write(
                self.style.SUCCESS(f"✓ Cached {notifications.count()} notifications")
            )
            ...
            self.stdout.write(
                self.style.SUCCESS(f"✓ Cached unread count: {unread_count}")
            )

            self.stdout.write(
                self.style.SUCCESS(
                    f"Successfully initialized cache for user ID {user_id}"
                )
            )
