...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            nargs="?",
            default=None,
            help="Optional user ID to check",
        )
    ...
    def handle(self, *args, **options):
        ...
        if user_id:
            ...
            if not users_with_notifications:
                self.stdout.write(
                    self.style.WARNING("No users with notifications found")
                )
                return

            self.stdout.write(
                f"Found {users_with_notifications.count()} users with notifications"
            )
    ...
    def check_user_cache(self, user_id):
        ...
        db_notifications = Notification.objects.filter(recipient_id=user_id).order_by(
            "-created_at"
        )
        ...
        if cached_notifications is not None:
            try:
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        f"Notification cache exists! Contains {num_cached} notifications"
                    )
                )
                ...
                if num_cached == num_db:
                    self.stdout.write(
                        self.style.SUCCESS(
                            f"Cache count matches database count: {num_db}"
                        )
                    )
                else:
                    self.stdout.write(
                ...
                self.stdout.write(
                    self.style.ERROR(f"Error accessing cached notifications: {str(e)}")
                )
        else:
        ...
        if cached_unread is not None:
            self.stdout.write(
                self.style.SUCCESS(f"Unread count is cached: {cached_unread}")
            )
            if cached_unread == db_unread:
                self.stdout.write(
                    self.style.SUCCESS(
                        f"Cached unread count matches database: {db_unread}"
                    )
                )
            else:
                self.stdout.write(
