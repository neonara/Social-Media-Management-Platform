...
class CreatePostView(APIView):
    ...
    def post(self, request, client_id=None):
        ...
        if not client_id:
            return Response(
                {"error": "Client ID is required."}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        if request.user.is_community_manager:
            if not client.assigned_communitymanagerstoclient.filter(
                id=request.user.id
            ).exists():
                return Response(
        ...
        if serializer.is_valid():
            try:
                ...
                scheduled_for = (
                    post.scheduled_for
                )  # This should now be a datetime object
    ...
    def validate_file(self, file):
        ...
        if file.size > max_size:
            raise ValidationError(
                f"File too large: {file.name} ({file.size/1024/1024:.1f}MB)"
            )
    ...
    def get(self, request):
        ...
        if request.user.is_client:
            ...
            posts = Post.objects.filter(
                ...
                | models.Q(  # Posts they created
                    client__assigned_communitymanagerstoclient=request.user
                )  # Posts for their assigned clients
            ).distinct()

        elif request.user.is_moderator:
            # Moderators see posts from their assigned CMs + posts for their assigned clients
            posts = Post.objects.filter(
                models.Q(creator__in=request.user.assigned_communitymanagers.all())
                | models.Q(  # Posts from their assigned CMs
                    client__assigned_moderator=request.user
                )
                | models.Q(  # Posts for their assigned clients
                    creator=request.user
                )  # Posts they created themselves (if any)
            ).distinct()
...
class ListPostsView(APIView):
    ...
    def get(self, request):
        ...
        bypass_cache = (
            True  # request.query_params.get('bypassCache', 'false').lower() == 'true'
        )
        ...
        if cached_data is None:
            ...
            if request.user.is_client:
                ...
                posts = Post.objects.filter(
                    ...
                    )  # All posts for their assigned clients
                ).distinct()
...
class UpdatePostView(APIView):
    ...
    def get(self, request, post_id):
        try:
            post = (
                Post.objects.select_related("client", "creator", "feedback_by")
    ...
    def get(self, request):
        ...
        if cached_data is None:
            ...
            assigned_clients = User.objects.filter(
                is_client=True, assigned_communitymanagerstoclient=user
            )

            # Get all posts for assigned clients (regardless of creator)
            posts = (
                Post.objects.filter(client__in=assigned_clients)
                .select_related("client", "creator")
                .prefetch_related("media")
            )
...
class FetchDraftsView(APIView):
    ...
    def get(self, request):
        ...
        if cached_data is None:
            ...
            drafts = (
                Post.objects.filter(filter_conditions)
                .distinct()
                .order_by("-created_at")
            )
...
class ApprovePostView(APIView):
    ...
    def patch(self, request, post_id):
        ...
        override_client = request.data.get(
            "override_client", False
        )  # For moderator override
        ...
        if post.client == request.user:
            ...
            if override_client or post.is_client_approved:
                ...
                approval_message = (
                    f"Post '{post.title}' validated by moderator and scheduled."
                )
            else:
        ...
        if (
            post.client
            and post.client != request.user
            and (request.user.is_moderator or request.user.is_administrator)
        ):
        ...
        if (
            request.user == post.client
            and post.creator
            and post.creator.is_moderator
            and post.creator != post.client.assigned_moderator
        ):
        ...
        if (
            request.user == post.client
            and post.client.assigned_moderator
            and post.client.assigned_moderator != post.creator
        ):
            moderator_message = (
                f"Post '{post.title}' approved by client. Waiting for your validation."
            )
            if feedback:
    ...
    def patch(self, request, post_id):
        ...
        if post.status == "scheduled":
            can_reject_scheduled = (
                request.user == post.client
                or request.user.is_administrator  # The client
                or (  # Admin or super admin
                    request.user.is_moderator
                    and post.client.assigned_moderator == request.user
                )  # Assigned moderator
            )
    ...
    def get(self, request):
        ...
        if cached_data is None:
            if request.user.is_client:
                # Clients see their own pending posts
                pending_posts = (
                    Post.objects.filter(client=request.user, status="pending")
                    .select_related("creator", "client")
                    .prefetch_related("media")
                )
                ...
                # CMs see pending posts for their assigned clients
                assigned_clients = User.objects.filter(
                    is_client=True, assigned_communitymanagerstoclient=request.user
                )
                pending_posts = (
                    Post.objects.filter(client__in=assigned_clients, status="pending")
                    .select_related("creator", "client")
                    .prefetch_related("media")
                )
            ...
            serializer = PostSerializer(
                pending_posts, many=True, context={"request": request}
            )
            cached_data = serializer.data
...
class PostFeedbackView(APIView):
    ...
    def get(self, request, post_id):
        ...
        if not (
            ...
            or post.creator == request.user
            or request.user.is_moderator
            or (
                request.user.is_community_manager
                and post.client
                and post.client.assigned_communitymanagerstoclient.filter(
                    id=request.user.id
                ).exists()
            )
        ):
    ...
    def get(self, request, post_id):
        ...
        try:
            ...
            cm_can_see_mod_post = (
                request.user.is_community_manager
                and post.creator
                and post.creator == request.user.assigned_moderator
                and post.client
                and post.client.assigned_communitymanagerstoclient.filter(
                    id=request.user.id
                ).exists()
            ...
            if not (
                post.is_user_assigned(request.user)
                or request.user.is_moderator
                or request.user.is_administrator
                or post.creator == request.user
                or cm_assigned_to_client
                or cm_can_see_mod_post
            ):
                return Response(
...
class ModeratorValidatePostView(APIView):
    ...
    def patch(self, request, post_id):
        ...
        if (
            (request.user.is_administrator or request.user.is_superadministrator)
            and post.client
            and post.client.assigned_moderator
            and post.client.assigned_moderator != request.user
            and post.client.assigned_moderator != post.creator
            ...
            admin_validation_message = (
                f"Post '{post.title}' validated by admin and scheduled."
            )
            if feedback:
