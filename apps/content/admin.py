...
class ContentAdminSite(admin.ModelAdmin):
    ...
    def get_urls(self):
        urls = super().get_urls()
        custom_urls = [
            path('content-stats/', self.admin_site.admin_view(self.content_stats_view), name='content_stats'),
        ]
    ...
    def content_stats_view(self, request):
        ...
        # Status breakdown
        status_stats = Post.objects.values('status').annotate(count=Count('id')).order_by('-count')
        ...
        if published_posts.exists():
            ...
            for post in published_posts.filter(published_at__isnull=False):
                if post.published_at and post.created_at:
                    workflow_time = (post.published_at - post.created_at).total_seconds() / 3600  # in hours
        ...
        top_clients = Post.objects.values(
        ...
        client_approval_rate = (approved_posts / total_pending_or_higher * 100) if total_pending_or_higher > 0 else 0
        moderator_validation_rate = (validated_posts / total_pending_or_higher * 100) if total_pending_or_higher > 0 else 0
        rejection_rate = (rejected_posts / total_pending_or_higher * 100) if total_pending_or_higher > 0 else 0
        ...
        # Recent activity
        recent_posts = Post.objects.select_related('creator', 'client').order_by('-updated_at')[:10]
...
class MediaAdmin(admin.ModelAdmin):
    model = Media
    list_display = ('name', 'type', 'creator', 'file_size_display', 'uploaded_at', 'posts_count')
    ...
    def posts_count(self, obj):
        ...
        if count > 0:
            url = reverse('admin:content_post_changelist') + f'?media__id__exact={obj.id}'
...
class PostAdmin(ContentAdminSite):
    model = Post
    list_display = (
        'title', 'creator', 'client', 'status', 'created_at', 'updated_at', 
        'last_edited_by', 'scheduled_for', 'workflow_status', 'days_since_creation'
    )
    list_filter = (
        'status', 'created_at', 'updated_at', 'is_client_approved', 
        'is_moderator_validated', 'creator', 'client', 'last_edited_by'
    )
    search_fields = ('title', 'description', 'creator__email', 'client__email')
    readonly_fields = (
        'created_at', 'updated_at', 'workflow_timeline', 'change_history',
        'time_in_workflow', 'total_revisions'
    )
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        for status_choice in Post.STATUS_CHOICES:
            ...
            status_counts[status_choice[1]] = self.get_queryset(request).filter(status=status_key).count()
        
        # Recent activity (last 7 days)
        last_7_days = timezone.now() - timedelta(days=7)
        recent_posts = self.get_queryset(request).filter(created_at__gte=last_7_days).count()
        
        # Posts needing attention
        posts_needing_attention = self.get_queryset(request).filter(
            Q(status='pending') & Q(created_at__lt=timezone.now() - timedelta(days=3))
        ).count()
        
        # Workflow efficiency
        avg_approval_time = self.get_queryset(request).filter(
            client_approved_at__isnull=False
        ).aggregate(
            avg_time=Avg(F('client_approved_at') - F('created_at'))
        )['avg_time']
        ...
        extra_context.update({
            ...
            'recent_posts': recent_posts,
            'posts_needing_attention': posts_needing_attention,
            'avg_approval_time': avg_approval_time.days if avg_approval_time else None,
    ...
    fieldsets = (
        ...
        ('Workflow Status', {
            'fields': (
                'is_client_approved', 'is_moderator_validated',
        ...
        ('Tracking Information', {
            'fields': (
                ...
                'workflow_timeline', 'change_history', 'time_in_workflow', 'total_revisions'
            ),
            'classes': ('collapse',)
        }),
        ('Workflow Timestamps', {
            'fields': (
                'client_approved_at', 'client_rejected_at',
                'moderator_validated_at', 'moderator_rejected_at',
                'published_at'
            ),
            'classes': ('collapse',)
        })
    ...
    def workflow_status(self, obj):
        ...
        return format_html(
            ...
            color, obj.get_status_display(), workflow_info
        )
    ...
    def workflow_timeline(self, obj):
        ...
        if obj.client_approved_at:
            timeline.append(f"✓ Client Approved: {obj.client_approved_at.strftime('%Y-%m-%d %H:%M')}")
        elif obj.client_rejected_at:
            timeline.append(f"✗ Client Rejected: {obj.client_rejected_at.strftime('%Y-%m-%d %H:%M')}")
            
        if obj.moderator_validated_at:
            timeline.append(f"✓ Moderator Validated: {obj.moderator_validated_at.strftime('%Y-%m-%d %H:%M')}")
        elif obj.moderator_rejected_at:
            timeline.append(f"✗ Moderator Rejected: {obj.moderator_rejected_at.strftime('%Y-%m-%d %H:%M')}")
            
        if obj.published_at:
            timeline.append(f"📢 Published: {obj.published_at.strftime('%Y-%m-%d %H:%M')}")
    ...
    def change_history(self, obj):
        ...
        if obj.moderator_validated_at and obj.client_approved_at:
            moderator_response_time = obj.moderator_validated_at - obj.client_approved_at
            changes.append(f"Moderator response time: {moderator_response_time.days} days")
        ...
        return format_html('<br>'.join(changes)) if changes else "No significant changes"
    ...
    def get_queryset(self, request):
        ...
        return qs.select_related('creator', 'client', 'last_edited_by', 'feedback_by', 'platform_page')
...
def get_urls_with_stats():
    ...
    custom_urls = [
        path('content/stats/', PostAdmin(Post, admin.site).content_stats_view, name='content_stats'),
    ]
    return custom_urls + urls
