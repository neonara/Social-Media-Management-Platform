...
Use ONLY valid JSON, no markdown."""
    ...
    def _build_hashtag_prompt(
        self, caption: str, platform: str, count: int, industry: Optional[str]
    ) -> str:
...
No markdown, just JSON."""
    ...
    def predict_engagement(self, post_data: Dict) -> Dict:
        ...
        try:
            ...
            caption_length = post_data.get(
                "caption_length", len(caption) if caption else 100
            )
            hashtag_count = post_data.get("hashtag_count", 10)
            time_of_day = post_data.get(
                "time_of_day", "12:00"
...
{{
  ...
  "best_time": null or "<suggest ONE specific time like '9:00 AM' or '6:30 PM' only if significantly better>"
            ...
            return {
                ...
                "reasoning": prediction_data.get(
                    "reasoning", "Post analysis by Gemini"
                ),
