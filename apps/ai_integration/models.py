...
class HashtagPerformance(models.Model):
    ...
    usage_frequency = models.IntegerField(default=0, help_text="Times used in sample data")
    reach_estimate = models.IntegerField(default=0, help_text="Estimated reach (synthetic)")
...
class EngagementForecast(models.Model):
    ...
    hashtag_count = models.IntegerField(
        help_text="Number of hashtags in the post"
    )
...
class ModelMetrics(models.Model):
    ...
    r2_score = models.FloatField(
        help_text="R² score on training data"
    )
    ...
    training_data_type = models.CharField(
        max_length=20,
        choices=TrainingData.DATA_TYPE_CHOICES,
        default='synthetic'
    )
    created_at = models.DateTimeField(auto_now_add=True)
