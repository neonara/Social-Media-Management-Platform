...
def suggest_hashtags(request):
    ...
    try:
        ...
        if not caption:
            return Response(
                {"error": "caption is required"}, status=status.HTTP_400_BAD_REQUEST
            )

        if not caption.strip():
            return Response(
                {"error": "caption cannot be empty"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        result = gemini.analyze_caption_for_hashtags(
            caption=caption, platform=platform, count=count, industry=industry
        )
...
def detect_campaign_theme(request):
    ...
    try:
        ...
        if not isinstance(captions, list):
            return Response(
                {"error": "captions must be a list"}, status=status.HTTP_400_BAD_REQUEST
            )
...
def generate_content_by_mood(request):
    ...
    try:
        ...
        result = gemini.generate_content_by_mood(
            topic=topic, mood=mood, tone=tone, platform=platform, count=count
        )
...
def rewrite_caption_by_mood(request):
    ...
    try:
        ...
        result = gemini.rewrite_caption_by_mood(
            caption=caption, mood=mood, tone=tone, platform=platform
        )
...
class EngagementPredictionView(APIView):
    ...
    def post(self, request):
        try:
            ...
            if (
                "caption" in serializer.validated_data
                and serializer.validated_data["caption"]
            ):
            ...
            if post_id:
                try:
                    ...
                    EngagementForecast.objects.update_or_create(
                        post=post,
                        defaults={
                            "caption_length": serializer.validated_data[
                                "caption_length"
                            ],
                            "hashtag_count": serializer.validated_data["hashtag_count"],
                            "time_of_day": serializer.validated_data["time_of_day"],
                            ...
                            "platform": serializer.validated_data["platform"],
                            "media_type": serializer.validated_data["media_type"],
                            "brand_sentiment": serializer.validated_data[
                                "brand_sentiment"
                            ],
                            "predicted_engagement_score": prediction.get(
                                "predicted_engagement_score",
                                prediction.get("score", 50),
                            ),
                            "engagement_level": prediction.get(
                                "engagement_level", prediction.get("level", "MEDIUM")
                            ),
                            "confidence_score": prediction.get(
                                "confidence_score", prediction.get("confidence", 75)
                            ),
                        },
                    )
...
def get_optimal_posting_times(request):
    ...
    try:
        ...
        times = OptimalPostingTime.objects.filter(platform=platform).order_by(
            "-engagement_score"
        )[:10]
        ...
        return Response(
            {"platform": platform, "optimal_times": optimal_times},
