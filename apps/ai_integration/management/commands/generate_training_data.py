...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if clear_existing:
            deleted_count, _ = TrainingData.objects.filter(data_type='synthetic').delete()
            self.stdout.write(
                self.style.SUCCESS(f'✓ Cleared {deleted_count} existing synthetic training samples')
            )
        ...
        self.stdout.write(
            self.style.SUCCESS(f'✓ Successfully generated {samples} synthetic training samples!')
        )
    ...
    def _generate_synthetic_data(self, num_samples):
        ...
        optimal_times = {
            'instagram': [(1, 11), (1, 19), (2, 11), (3, 9), (4, 17), (5, 10)],  # Tue 11am, Tue 7pm, etc
        ...
        for _ in range(num_samples):
            ...
            is_optimal_time = (day_of_week, time_of_day) in optimal_times.get(platform, [])
            if is_optimal_time:
    ...
    def _display_statistics(self):
        ...
        # By platform
        platforms = TrainingData.objects.filter(data_type='synthetic').values('platform').distinct()
        for p in platforms:
            platform = p['platform']
            count = TrainingData.objects.filter(data_type='synthetic', platform=platform).count()
            ...
            if avg_engagement:
                ...
                self.stdout.write(f'   • {platform.title()}: {count} samples (avg engagement: {avg:.1f}/100)')
        
        # By media type
        self.stdout.write('\n   By Media Type:')
        media_types = TrainingData.objects.filter(data_type='synthetic').values('media_type').distinct()
        for m in media_types:
            media_type = m['media_type']
            count = TrainingData.objects.filter(data_type='synthetic', media_type=media_type).count()
            avg_engagement = TrainingData.objects.filter(
                data_type='synthetic',
                media_type=media_type
            ).values_list('engagement_score', flat=True)
            if avg_engagement:
                avg = sum(avg_engagement) / len(avg_engagement)
                self.stdout.write(f'   • {media_type.title()}: {count} samples (avg engagement: {avg:.1f}/100)')
