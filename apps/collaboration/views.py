        ...
        if serializer.validated_data.get('room_type') == 'direct':
            ...
            if len(members) == 1:  # Should be one other user
                ...
                existing_room = ChatRoom.objects.filter(
                    ...
                    is_active=True
                ).filter(members=other_user).annotate(member_count=Count('members')).filter(member_count=2).first()
...
class SendMessageView(APIView):
    ...
    def post(self, request):
        ...
        try:
            room = ChatRoom.objects.get(id=room_id, members=request.user, is_active=True)
    ...
    def get(self, request):
        ...
        try:
            ...
            room = ChatRoom.objects.filter(
                ...
                members=request.user,
                is_active=True
            ).filter(members=other_user_id).annotate(member_count=Count('members')).filter(member_count=2).first()

            if room:
                serializer = ChatRoomSerializer(room)
                return Response(serializer.data)
            else:
                return Response({'error': 'No direct message room found'}, status=status.HTTP_404_NOT_FOUND)
