...
class AssignModeratorSerializer(serializers.Serializer):
    ...
    def validate_moderator_id(self, value):
        try:
            ...
            raise serializers.ValidationError(
                "Moderator does not exist or is not a valid moderator."
            )
        return value
...
class AssignCommunityManagerToClientSerializer(serializers.Serializer):
    ...
    def validate_cm_id(self, value):
        try:
            ...
            raise serializers.ValidationError(
                "A community manager with this ID does not exist."
            )
        return value
...
class AssigncommunityManagerstoModeratorsSerializer(serializers.Serializer):
    ...
    def validate_cm_id(self, value):
        try:
            ...
            raise serializers.ValidationError(
                "Community Manager does not exist or is not a valid CM."
            )
        return value
...
class RemoveCMsFromClientSerializer(serializers.Serializer):
    ...
    def validate_community_manager_ids(self, values):
        if not values:
            raise serializers.ValidationError(
                "Please provide a list of community manager IDs to remove."
            )
        for cm_id in values:
            try:
                ...
                raise serializers.ValidationError(
                    f"Community manager with ID {cm_id} not found."
                )
        return values
        ...
        if user_image and isinstance(user_image, str):
            ...
            if self.instance and self.instance.user_image:
                ...
                if user_image.startswith("/media"):
                    ...
                    user_image == self.instance.user_image.url
                    or user_image in self.instance.user_image.url
                ):
                    data.pop("user_image", None)  # Remove from validation data
                else:
