...
class UserActivityConsumer(AsyncWebsocketConsumer):
        ...
        if self.user_id:
            ...
            await self.channel_layer.group_send(
                self.group_name,
                {
                    ...
                    "user": {
                        "id": self.user_id,
                        "name": f"{self.scope['user'].first_name} {self.scope['user'].last_name}".strip()
                        if self.scope["user"].first_name or self.scope["user"].last_name
                        else self.scope["user"].email,
                        "email": self.scope["user"].email,
                        "profilePicture": self.scope["user"].user_image.url
                        if self.scope["user"].user_image
                        else "",
                    },
                },
            )
    ...
    async def disconnect(self):
        if self.user_id:
            ...
            await self.channel_layer.group_send(
                self.group_name, {"type": "user_left", "userId": self.user_id}
            )
    ...
    async def user_joined(self, event):
        ...
        if self.user_id != event["user"]["id"]:
            await self.send(
                text_data=json.dumps({"type": "user-joined", "user": event["user"]})
            )
    ...
    async def user_left(self, event):
        ...
        if self.user_id != event["userId"]:
            await self.send(
                text_data=json.dumps({"type": "user-left", "userId": event["userId"]})
            )
...
class UserDataConsumer(AsyncWebsocketConsumer):
    ...
    async def connect(self):
        ...
        if self.user.is_anonymous:
            print(
                f"UserDataConsumer: Rejecting anonymous user connection from {self.scope.get('client', ['unknown'])[0]}"
            )
            await self.close(code=1008)  # Policy Violation - authentication required
            return

        print(
            f"UserDataConsumer: Accepting connection for user {self.user.id} ({self.user.email})"
        )
            ...
            text_data=json.dumps(
                {
                    ...
                    "assignment_type": event.get(
                        "assignment_type"
                    ),  # 'moderator_client', 'cm_moderator', 'cm_client'
