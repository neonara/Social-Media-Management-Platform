...
from .serializers import (
    ...
    AssignModeratorSerializer,
    GetUserSerializer,
    UserLoginSerializer,
    CreateUserSerializer,
    FirstTimePasswordChangeSerializer,
    AssigncommunityManagerstoModeratorsSerializer,
    RemoveCMsFromClientSerializer,
...
class IsAdminOrSuperAdmin(BasePermission):
    ...
    def has_permission(self, request, view):
        return request.user.is_authenticated and (
            request.user.is_superadministrator or request.user.is_administrator
        )
...
class IsModeratorOrAdmin(BasePermission):
    ...
    def has_permission(self, request, view):
        return request.user.is_authenticated and (
            ...
            or request.user.is_superadministrator
        )
...
class ClientFetchModeratorAndCMsView(APIView):
    ...
    def get(self, request):
        ...
        assigned_cms = moderator.assigned_communitymanagers.filter(
        ...
        data = {
            "moderator": {
                ...
                "user_image": moderator.user_image.url
                if moderator.user_image
                else None,
            },
            "community_managers": [
                {
...
class AssignedModeratorsAndClientsView(APIView):
    ...
    def get(self, request):
        ...
        assigned_moderators = User.objects.filter(
            assigned_communitymanagers=user, is_moderator=True
        )

        # Fetch assigned clients
        assigned_clients = User.objects.filter(
            assigned_communitymanagerstoclient=user, is_client=True
        )
...
class GetUserByIdView(APIView):
    ...
    def get(self, request, user_id):
        ...
        bypass_cache = (
            request.query_params.get("bypassCache", "false").lower() == "true"
        )
...
class ClientAssignedCommunityManagersView(APIView):
    ...
    permission_classes = [
        IsAuthenticated
    ]  # You might want to add more specific permissions
                    ...
                    "assigned_clients": [
                        {
                            ...
                            "full_name": client.full_name,
                            "email": client.email,
                            "user_image": client.user_image.url
                            if client.user_image
                            else None,
                        }
                        for client in assigned_clients
                    ]
...
class EligibleCMsForClient(APIView):
    ...
    def get(self, request, client_id):
        ...
        if not client.assigned_moderator:
            return Response(
                {"detail": "This client has no assigned moderator."},
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            try:
                ...
                notify_user_created(
                    user_id=user.id, created_by_id=request.user.id, user_data=user_data
                )
...
class CreateCMView(APIView):
    permission_classes = [
        IsAuthenticated,
        IsModerator,
    ]  # Only authenticated Moderators can access
...
class UserLoginView(APIView):
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            response_data = {
                k: v for k, v in serializer.validated_data.items() if k != "remember"
            }
            ...
            logger.info(
                f'Successful login for user: {serializer.validated_data.get("email")} '
...
class LogoutUserView(APIView):
    ...
    def post(self, request):
        ...
        logger = logging.getLogger("security")
        logger.info(
            f'Successful logout for user ID: {user_id} from IP: {request.META.get("REMOTE_ADDR")}'
        )
...
class PasswordResetRequestView(generics.GenericAPIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            return Response(
                {"email": "User with this email does not exist."}, status=400
            )
        except Exception as e:
...
class AssignCMToClientView(APIView):
    ...
    def put(self, request, client_id):
        ...
        if not client.assigned_moderator:
            return Response(
                {
                    "error": "This client has no assigned moderator. Assign a moderator first."
                },
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        if serializer.is_valid():
            ...
            try:
                community_manager = User.objects.get(
                    id=cm_id, is_community_manager=True
                )
            except User.DoesNotExist:
                return Response(
            ...
            if not client.assigned_moderator.assigned_communitymanagers.filter(
                id=community_manager.id
            ).exists():
                return Response(
                    {
                        "error": "This community manager is not assigned to the client's moderator."
                    },
                    status=status.HTTP_400_BAD_REQUEST,
                )
            ...
            return Response(
                {
                    "message": f"Community Manager {community_manager.email} assigned to client {client.email}."
                },
                status=status.HTTP_200_OK,
            )
    ...
    def delete(self, request, moderator_id, cm_id):
        try:
            ...
            if cm_to_remove in moderator.assigned_communitymanagers.all():
                ...
                return Response(
                    {
                        "message": "Community Manager unassigned from Moderator and associated clients."
                    },
                    status=status.HTTP_200_OK,
                )
            else:
                return Response(
                    {
                        "error": "This Community Manager is not assigned to this Moderator."
                    },
                    status=status.HTTP_400_BAD_REQUEST,
                )
        except User.DoesNotExist:
    ...
    def delete(self, request, client_id):
        try:
            ...
            if client.assigned_moderator is None:
                return Response(
                    {"message": "No moderator assigned to this client."},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            ...
            return Response(
                {
                    "message": "Moderator unassigned from client and associated community manager assignments removed."
                },
                status=status.HTTP_200_OK,
            )
        except User.DoesNotExist:
...
class RemoveCommunityManagersView(generics.UpdateAPIView):
    ...
    permission_classes = [
        IsModeratorOrAdmin
    ]  # Allow both moderators and admins to manage client relationships
    ...
    def update(self, request, *args, **kwargs):
        ...
        invalid_ids = set(cm_ids_to_remove) - set(
            cms_to_remove.values_list("id", flat=True)
        )
        if invalid_ids:
            return Response(
                {
...
class AssignedClientsView(APIView):
    ...
    def get(self, request):
        ...
        if user.is_community_manager:
            ...
            clients = User.objects.filter(assigned_moderator=user, is_client=True)
        else:
            return Response(
                {"error": "You do not have permission to access this resource."},
                status=status.HTTP_403_FORBIDDEN,
...
class AssignModeratorToClientView(APIView):
    ...
    def put(self, request, client_id):
        ...
        if serializer.is_valid():
            ...
            return Response(
                {
                    "message": f"Moderator {moderator.email} assigned to client {client.email}."
                }
            )
...
class AssignCommunityManagerToModeratorView(APIView):
    ...
    def put(self, request, moderator_id):
        ...
        if serializer.is_valid():
            ...
            return Response(
                {
                    "message": f"Community Manager {cm.email} assigned to Moderator {moderator.email}."
                }
            )
    ...
    def delete(self, request, moderator_id, cm_id):
        try:
            ...
            if cm_to_remove in moderator.assigned_communitymanagers.all():
                ...
                return Response(
                    {
                        "message": "Community Manager unassigned from Moderator. Posts remain associated with the client."
                    },
                    status=status.HTTP_200_OK,
                )
...
class RemoveModeratorFromClientView(APIView):
    ...
    def delete(self, request, client_id):
        try:
            ...
            return Response(
                {
                    "message": "Moderator unassigned from client. Posts remain associated with the client."
                },
                status=status.HTTP_200_OK,
            )
...
class AdminDeleteUserView(APIView):
    permission_classes = [
        IsAdminOrSuperAdmin
    ]  # Only administrators or superadministrators can access this view
    ...
    def delete(self, request, user_id):
        try:
            ...
            )  # Store email before deletion for the response message
...
class ValidateTokenView(APIView):
    ...
    def post(self, request):
        try:
            ...
            logger = logging.getLogger("security")
            logger.warning(
                f'Token validation failed from IP: {request.META.get("REMOTE_ADDR")} '
                f"Error: {str(e)}"
            )
...
class ValidateRoleView(APIView):
    ...
    def post(self, request):
        try:
            ...
            logger = logging.getLogger("security")
            logger.info(
                f"Role validation for user: {user.email} "
            ...
            logger = logging.getLogger("security")
            logger.warning(
                f'Role validation failed from IP: {request.META.get("REMOTE_ADDR")} '
                f"Error: {str(e)}"
            )
