...
def build_user_data(user):
    ...
    if (
        user.is_client
        and hasattr(user, "assigned_moderator")
        and user.assigned_moderator
    ):
    ...
    if user.is_moderator:
        ...
        data["assigned_communitymanagers"] = (
            ", ".join([cm.full_name for cm in assigned_cms]) if assigned_cms else None
        )
...
def notify_user_data_updated(
    user_id, updated_by_id, fields_changed=None, user_data=None
):
    """
...
def notify_assignment_changed(
    user_id, target_id, assignment_type, action, updated_by_id, data=None
):
    """
